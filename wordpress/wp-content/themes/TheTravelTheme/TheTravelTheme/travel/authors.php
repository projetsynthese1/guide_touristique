		...
		if( $count > 0){
			...
			for($i=1; $i<= $count; $i++){
				...
				if($id){
					...
					$output .= '<li><div class="gravatar"><a href="'.get_author_posts_url( $id, get_the_author_meta('user_nicename',$id) ).'">'.get_avatar( get_the_author_meta('user_email',$id), '60' ).'</a></div><div class="author_name"><a href="'.get_author_posts_url( $id, get_the_author_meta('user_nicename',$id) ).'">'.get_the_author_meta('user_nicename',$id).'</a></div><div class="author_desc">'.$desc.'</div></li>';
				}
			}
			$output .= '</ul>';
		}
